...
namespace BMS
{
	public partial class frmError : _Forms
	{
		...
		void LoadInfoSearch()
		{
			...
			dt = TextUtilsStock.Select($"SELECT * FROM [ShiStock].[dbo].[Error]");
		...
		private void btnEdit_Click(object sender, EventArgs e)
		{
			...
			if (id == 0) return;
			ErrorModel model = (ErrorModel)ErrorBO.Instance.FindByPK(id);
			frmErrorDetail frm = new frmErrorDetail();
		...
		private void btnDel_Click(object sender, EventArgs e)
		{
			...
			if (MessageBox.Show(String.Format("Bạn có chắc muốn xóa [{0}] không?", strName), TextUtils.Caption, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
			{
				try
				{
					ErrorBO.Instance.Delete(Convert.ToInt32(strID));
					LoadInfoSearch();
				}
				catch
				{
