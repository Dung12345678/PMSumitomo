...
namespace BMS
{
	public partial class frmImportAssemblyDateExcel : _Forms
	{
		private DataSet ds;
		public frmImportAssemblyDateExcel()
		{
			InitializeComponent();
		}
		DateTime start;
		...
		private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			for (int i = 0; i < rowCount; i++)
			{
				try
				{
					...
					{
						DeliveryDate = Lib.ToDate4(grvData.GetRowCellValue(i, "DATEF2"));
					}

					AssemblyDate = Lib.ToDate3(grvData.GetRowCellValue(i, "F3"));//Ngày lắp ráp	////ngày chốt
					if (Lib.ToDate3(grvData.GetRowCellValue(i, "F3")) == new DateTime(1950, 1, 1))
					{
						AssemblyDate = Lib.ToDate2(grvData.GetRowCellValue(i, "DATEF3"));
					}
