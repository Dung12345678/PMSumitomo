...
namespace BMS
{
	public partial class frmReduceSpeedDetailH : _Forms
	{
		public SpeedReducerModel _SpeedReducerModel = new SpeedReducerModel();
		public frmReduceSpeedDetailH()
		{
			InitializeComponent();
		}
		...
		private bool ValidateForm()
		{
			...
			else
			{
				...
				{
					dt = LibIE.Select("select top 1 1 from SpeedReducer where Name = '" + txtName.Text.Trim() + "' and ID <> " + _SpeedReducerModel.ID);
				}
				else
				{
					dt = LibIE.Select("select top 1 1 from SpeedReducer where Name = '" + txtName.Text.Trim() + "'");
				}
				if (dt != null)
				{
					if (dt.Rows.Count > 0)
		...
		bool saveData()
		{
			...
			_SpeedReducerModel.TimeGetVongQuay = TextUtils.ToDecimal(NTimeGetVongQuay.Value);
		...
		void loadData()
		{
			...
			NTimeGetVongQuay.Value = _SpeedReducerModel.TimeGetVongQuay;
