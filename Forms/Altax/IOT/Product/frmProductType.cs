...
namespace BMS
{
    public partial class frmProductType : _Forms
    {
        private bool _isAdd;
        ...
        private bool checkValid()
        {
            ...
            else
            {
                ...
                if (!_isAdd)
                {
                    ...
                    dt = TextUtils.Select("select top 1 1 from ProductType WITH(NOLOCK) where ProductTypeCode = '" + txtCode.Text.Trim() + "' and ID <> " + strID);
                }
                else
                {
                    dt = TextUtils.Select("select top 1 1 from ProductType WITH(NOLOCK) where ProductTypeCode = '" + txtCode.Text.Trim() + "'");
                }
                if (dt != null)
                {
                    if (dt.Rows.Count > 0)
        ...
        private void loadData()
        {
            try
            {
                DataTable tbl = TextUtils.Select("SELECT * FROM ProductType WITH(NOLOCK)");
