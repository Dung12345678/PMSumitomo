...
namespace BMS
{
    public partial class frmWorkingDetail : _Forms
    {
        public WorkingModel WorkingModel = new WorkingModel();
        public int GroupID;
        public int StepID;
        public frmWorkingDetail()
        {
            InitializeComponent();
        }
        void loadGroup()
        ...
        void loadStep()
        {
            ...
            if (groupID > 0)
            {
                dt = TextUtils.Select(string.Format("SELECT * FROM dbo.WorkingStep WHERE ProductGroupID = {0} ORDER BY SortOrder", groupID));
            }
        ...
        private bool ValidateForm()
        {
            ...
            if (WorkingModel.ID > 0)
            {
                sql = $"select ID from Working w where SortOrder='{txtSortOrder}' and w.ID <> {WorkingModel.ID} and WorkingStepID={TextUtils.ToInt(cboStep.EditValue)}";
            }
            else
            {
                sql = $"select ID from Working where SortOrder='{txtSortOrder}' and WorkingStepID={TextUtils.ToInt(cboStep.EditValue)}";
            }
        ...
        bool saveData()
        {
            ...
            if (WorkingModel.ID > 0)
            {
                ...
                TextUtils.ExcuteSQL(string.Format("EXEC spUpdateMultiProductWorking {0}, 1", WorkingModel.ID));
            }
            else
            {
                WorkingModel.ID = (int)WorkingBO.Instance.Insert(WorkingModel);
                TextUtils.ExcuteSQL(string.Format("EXEC CreateProductWorking_ByWorkingID {0}", WorkingModel.ID));
            }
            return true;
        }
