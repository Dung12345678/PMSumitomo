...
namespace BMS
{
    public partial class frmWorking : _Forms
    {
        public frmWorking()
        {
            InitializeComponent();
        ...
        void loadStep()
        {
            ...
            if (groupID > 0)
            {
                dt = TextUtils.Select(string.Format("SELECT * FROM dbo.WorkingStep WHERE ProductGroupID = {0} AND (IsDeleted = 0 OR IsDeleted IS NULL) ORDER BY SortOrder", groupID));
            }
        ...
        void loadWorking()
        {
            ...
            if (chkAll.Checked)
            {
                dt = TextUtils.Select(string.Format($"select *,(CAST(ws.SortOrder as nvarchar)+'-'+cast(ws.WorkingStepCode as nvarchar)) as WorkingStepCodes, case when w.CheckValueType=1 then N'Số' when w.CheckValueType = 2 then N'Ký tự' when w.CheckValueType = 0 then ''  END as CheckValueTypes  from Working w left join WorkingStep ws on w.WorkingStepID = ws.ID Left join ProductGroup pg on ws.ProductGroupID = pg.ID where ProductGroupID = '{cboGroup.EditValue}' Order by w.SortOrder"));
            }
            else
            {
                if (stepID > 0)
                {
                    dt = TextUtils.Select(string.Format("SELECT *, case when w.CheckValueType=1 then N'Số' when w.CheckValueType = 2 then N'Ký tự' when w.CheckValueType = 0 then ''  END as CheckValueTypes  FROM dbo.Working w WHERE WorkingStepID = {0} ORDER BY SortOrder", stepID));
                }
            }
        ...
        {
            frmWorkingDetail frm = new frmWorkingDetail();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                TextUtils.ExcuteSQL(string.Format("EXEC spUpdateMultiProductWorking {0}, 2", id));
            }
            catch (Exception ex)
            {
        ...
        {
            frmCopyWorkingToGroup frm = new frmCopyWorkingToGroup();
            frm.GroupID = TextUtils.ToInt(cboGroup.EditValue);
        ...
        {
            frmCopyStep frm = new frmCopyStep();
            frm.GroupID = TextUtils.ToInt(cboGroup.EditValue);
            frm.WorkingStepID = TextUtils.ToInt(grvData.GetFocusedRowCellValue(colStepID));
            frm.GroupName = cboGroup.Text.Trim();
