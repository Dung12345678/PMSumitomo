...
namespace BMS
{
	public partial class frmProductCheckHistoryDetailPCAAltax : _Forms
	{
		public frmProductCheckHistoryDetailPCAAltax()
		{
			InitializeComponent();
		}
		void LoadInfoSearch()
		...
		private void btnExportExecl_Click(object sender, EventArgs e)
		{
			try
			{
				if (grvData.RowCount > 0)
				{
					...
					{
						TextUtils.ExportExcel(grvData, od.SelectedPath, string.Format("DanhSachSanPhamPCAAltax_{0}", DateTime.Now.ToString("yyyy_MM_dd_HH_mm_ss")));
					}
				}
			}
			catch (Exception ex)
