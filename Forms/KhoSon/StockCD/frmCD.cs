...
namespace BMS
{
	public partial class frmCD : _Forms
	{
		...
		void loadCbo()
		{
			DataTable dt = TextUtilsStock.Select("SELECT * FROM dbo.AssemblyStock WITH(NOLOCK)");
		...
		void loadStockCDDetail()
		{
			...
			txtTaktTime.Value = TextUtilsStock.ToDate3(_StockCD.TaktTime);
		...
		bool saveData()
		{
			...
			_StockCD.StockID = TextUtilsStock.ToInt(cboStockName.SelectedValue);
			_StockCD.TaktTime = txtTaktTime.Value;
			if (_StockCD.StockName.Trim().ToUpper().Contains("MOTOR"))
			{
			...
			if (_StockCD.ID > 0)
			{
				...
				TextUtilsStock.ExcuteSQL($"Update Location set CDName='{txtCDName.Text.Trim()}' where CDID={_StockCD.ID}");
			}
			else
			{
		...
		private bool ValidateForm()
		{
			...
			if (id > 0)
			{
				MessageBox.Show("Đã tồn tại vị trí", TextUtilsStock.Caption, MessageBoxButtons.OK, MessageBoxIcon.Stop);
				return false;
			}
			return true;
		}
