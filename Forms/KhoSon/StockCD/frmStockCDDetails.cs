...
namespace BMS
{
	public partial class frmStockCDDetails : _Forms
	{
		public AssemblyStockModel _Stock = new AssemblyStockModel();
		public frmStockCDDetails()
		{
			InitializeComponent();
		}
		...
		bool saveData()
		{
			...
			if (_Stock.ID > 0)
			{
				...
				TextUtilsStock.ExcuteSQL($"Update StockCD set StockName=N'{txtStockName.Text.Trim()}' where StockID={_Stock.ID}");
